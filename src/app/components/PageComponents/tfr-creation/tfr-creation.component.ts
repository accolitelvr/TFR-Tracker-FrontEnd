...
export class TfrCreationComponent implements OnInit {
  ...
  selectionObserver = {next:((milestoneToSelect:any)=>{this.milestoneManagerService.setSelected(milestoneToSelect)})}
  updateObserver = {next:(()=>{this.milestones=this.milestoneManagerService.getMilestones();
